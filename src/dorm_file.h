...
void search_std_no(LateApply *a[], int n, int x);  // 학번 입력하면 해당 데이터 불러오는 함수

void return_dorm(LateApply *a[], int n, int x); // 기숙사 리턴 시간을 입력받아 데이터에 저장하는 함수

int getStdID(LateApply *a[], int n); //학번 입력받는 함수